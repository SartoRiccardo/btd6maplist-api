...
async def test_read_rules(btd6ml_test_client, mock_discord_api):
    ...
    async with btd6ml_test_client.post("/auth?discord_token=test_token") as resp:
        ...
        assert not resp_data["maplist_profile"]["has_seen_popup"], "New user has seen popup"
    ...
    async with btd6ml_test_client.post("/auth?discord_token=test_token") as resp:
        resp_data = await resp.json()
        assert resp_data["maplist_profile"]["has_seen_popup"], "New user has still not seen popup"
